/**
 * Einmalige Migration des Lerninhalts aus dem HTML-Prototyp (`CASES`, `ANATOMY`,
 * `MEDIALIBRARY`) in die Datenbank. Texte werden wortgetreu uebernommen, inklusive
 * sourceStatus — keine Ergaenzung oder Umformulierung von Quellenangaben.
 * Alle Inhalte bleiben Status DRAFT, bis eine Reviewerin sie ueber die
 * Review-Oberflaeche freigibt.
 */
...
async function main() {
  ...
  console.log(`Seed abgeschlossen: ${CASES.length} Fälle, ${ANATOMY.length} Anatomie-Items, ${MEDIALIBRARY.length} Medien.`);
}
